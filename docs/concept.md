...
> **一行で言うと**: 運用中の自作マイクロサービス群をリアルタイム稼働状況で一般公開し、「ちゃんと動いている」信頼を見せて AI コンサルの問い合わせにつなげる公開ショーケース + lead-gen LP。

| 項目 | 内容 |
|---|---|
| ユーザー | サービスを見に来た一般の訪問者（潜在コンサル顧客 / SNS・検索流入） + 運用者（seiji、問い合わせに返信する側） |
| 解決する課題 | 「作ったが誰も知らない」の逆。実際に動いているサービス群を可視化して信頼を作り、AI 駆動開発のコンサル lead を獲得する |
| 提供価値 | リアルタイム稼働状況による「本当に動いている」信頼 + 週1ペースで作るメイカーの実績可視化 + 気軽に相談できる問い合わせ導線 |
| 現フェーズ | 企画（concept 初版） |
| 最終更新 | 2026-05-27 |
...
shipyard は、seiji が運用している自作マイクロサービス群を一般向けに公開するショーケースサイト兼ランディングページ。中核は「いま実際に動いているサービス一覧」をリアルタイムの稼働状況（up/down）付きで見せることで、「口先だけでなくちゃんと動いているものがある」という信頼を訪問者に与えること。その信頼を土台に「AI 駆動開発で速く作る／コンサル承ります」という打ち出しを行い、問い合わせフォーム（サイト内スレッド形式）でコンサル案件の lead を獲得する。
...
3. **提供価値・コンサルの打ち出しを読む**: 訪問者が LP で「何者か（週1ペースで AI 駆動開発）／できること／進め方」を理解する。
4. **問い合わせスレッドを立てる**: 訪問者がメールアドレス + 本文を入力して問い合わせを送信 → 不可視スパム対策を通過 → サイト内にスレッドが即生成・表示され、トークン URL で後から戻れる。
