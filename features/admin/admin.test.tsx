...
import { adminReply, adminClose, type AdminReplyDeps, type AdminCloseDeps } from "./service";
...
function replyDeps(thread: Thread | null, email = "user@example.com") {
  ...
  const inquirers = {
    findById: vi.fn(async () => (email ? { id: "inq_1", email, createdAt: NOW } : null)),
  } as unknown as InquirerRepo;
  const notifyReply = vi.fn(async () => ({ ok: true }));
  return { deps: { threads, messages, inquirers, notifyReply } as AdminReplyDeps, threads, messages, notifyReply };
}

describe("adminReply (U-2, U-E4, U-E5, U-P1)", () => {
  it("U-2: operator message 追加 + touchActivity + 返信通知", async () => {
    const { deps, threads, messages, notifyReply } = replyDeps(makeThread());
    const res = await adminReply("th_1", "ご返信です", deps);
    expect(res).toEqual({ ok: true });
    expect(messages.add).toHaveBeenCalledWith({ threadId: "th_1", sender: "operator", body: "ご返信です" });
    expect(threads.touchActivity).toHaveBeenCalledWith("th_1");
    expect(notifyReply).toHaveBeenCalledWith("user@example.com", "TOK");
  });

  it("U-P1: 返信通知に本文を渡さない（リンクのみ、SEC-001）", async () => {
    const { deps, notifyReply } = replyDeps(makeThread());
    await adminReply("th_1", "秘密の返信本文", deps);
    const args = JSON.stringify((notifyReply as ReturnType<typeof vi.fn>).mock.calls);
    expect(args).not.toContain("秘密の返信本文");
  });

  it("U-E4: 存在しない thread → 404", async () => {
    const { deps, messages } = replyDeps(null);
    expect(await adminReply("nope", "x", deps)).toEqual({ ok: false, status: 404 });
    expect(messages.add).not.toHaveBeenCalled();
  });

  it("U-E5: 通知 throw でも message 追加は成功（best-effort）", async () => {
    const { deps, notifyReply, messages } = replyDeps(makeThread());
    (notifyReply as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("resend down"));
...
describe("adminClose (U-3)", () => {
  ...
  it("存在しない thread → 404", async () => {
    const threads = { findById: vi.fn(async () => null), setStatus: vi.fn() } as unknown as ThreadRepo;
    expect(await adminClose("nope", { threads } as AdminCloseDeps)).toEqual({ ok: false, status: 404 });
  });
});
...
describe("replySchema (U-E3)", () => {
  it("空/超過 body は reject、正常は通す", () => {
    ...
    expect(adminReplySchema.safeParse({ body: "a".repeat(5001) }).success).toBe(false);
  });
});
...
describe("認可ゲート（U-E1, U-E2 — requireOperator がルートの gate）", () => {
  it("未認証 → 401 / allowlist 外 → 403（route が auth.status を返す）", async () => {
    ...
    expect(await requireOperator(async () => null)).toEqual({ ok: false, status: 401 });
    expect(await requireOperator(async () => ({ userId: "u", email: "x@example.com" }))).toEqual({
      ok: false,
      status: 403,
    });
    expect(
      (await requireOperator(async () => ({ userId: "u", email: "ops@example.com" }))).ok,
    ).toBe(true);
  });
});
...
describe("ThreadList (U-1, U-B1)", () => {
  it("U-1: スレッドをリンク + 状態タグで表示（順序は渡された通り）", () => {
    render(
      <ThreadList
        threads={[
          { id: "a", subject: "新しい問い合わせ", status: "open", lastActivityAt: NOW },
          { id: "b", subject: "完了済み", status: "closed", lastActivityAt: NOW },
        ]}
      />,
    );
    expect(screen.getByRole("link", { name: /新しい問い合わせ/ })).toHaveAttribute("href", "/admin/threads/a");
  ...
  it("U-B1: 0 件は EmptyState", () => {
    ...
    expect(screen.getByText(/まだお問い合わせはありません/)).toBeInTheDocument();
  });
});
