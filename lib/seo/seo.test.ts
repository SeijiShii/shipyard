...
describe("buildMetadata (U-1, U-E1, U-E2)", () => {
  it("U-1: title/description/canonical/og/twitter", () => {
    const m = buildMetadata({ title: "稼働状況", description: "今の様子", path: "/status" });
    expect(m.title).toBe("稼働状況 — shipyard");
    expect(m.description).toBe("今の様子");
    expect(m.alternates?.canonical).toBe("https://shipyard.test/status");
    expect((m.openGraph as { url?: string })?.url).toBe("https://shipyard.test/status");
  ...
  it("U-E2: noindex で index:false, follow:false", () => {
    const m = buildMetadata({ title: "スレッド", path: "/t/abc", noindex: true });
...
describe("robots (U-4)", () => {
  it("admin/api/t を disallow + sitemap を指す", () => {
    ...
    expect(rule?.disallow).toEqual(expect.arrayContaining(["/admin", "/api", "/t/"]));
...
describe("og.ogTitle (U-B1)", () => {
  it("長文は切り詰め、Unicode で崩れない", () => {
    ...
    expect(ogTitle(null)).toBe("shipyard");
    expect(ogTitle("   ")).toBe("shipyard");
